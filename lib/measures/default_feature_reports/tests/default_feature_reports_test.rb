...
class DefaultFeatureReportsTest < MiniTest::Test
  ...
  def test_district_heating_values
    test_name = 'district_heating_values'
    model_in_path = "#{File.dirname(__FILE__)}/example_district.osm"
    ...
    # make sure the report files exists
    report_csv = "#{run_dir(test_name)}/default_feature_reports.csv"
    report_json = "#{run_dir(test_name)}/default_feature_reports.json"
    assert(File.exist?(report_csv))
    assert(File.exist?(report_json))
    
    # Check CSV headers include our district heating water column
    csv_headers = CSV.read(report_csv, headers: true).headers
    assert_includes(csv_headers, 'DistrictHeatingWater:Facility(kWh)')

    # Check JSON timeseries definition includes our column
    json_data = JSON.parse(File.read(report_json))
    col_names = json_data['timeseries_csv']['column_names']
    assert_includes(col_names, 'DistrictHeatingWater:Facility(kWh)')

    # Check JSON reporting_periods has district_heating_water_kwh end use data
    rp = json_data['reporting_periods'].first
    end_uses = rp['end_uses']
    assert(end_uses.key?('district_heating_water_kwh'))
  end
end
